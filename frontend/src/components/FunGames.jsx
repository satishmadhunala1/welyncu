...
const FunGames = () => {
    ...
    return (
        <div className="bg-[#1A1A1E] border border-gray-800 rounded-xl p-6 md:p-8 shadow-2xl shadow-purple-900/10">
            ...
            <div className='relative overflow-hidden rounded-xl'>
                ...
                <AvatarCircles numPeople={99} avatarUrls={avatars} className="absolute bottom-3 right-3" />
            </div>
            ...
            <h2 className="text-2xl md:text-[28px] font-barlow font-semibold text-gray-100 flex flex-wrap justify-center items-center mt-6 gap-2">
                <span>Fun</span>
                <Highlighter action="highlight" color="#87CEFA" className="px-2">
                    Games
                </Highlighter>
                <span>For</span>
                <Highlighter color="#FF9800" className="px-2">
                    You
                </Highlighter>
            </h2>
            ...
            <div className='mt-8 grid grid-cols-2 md:grid-cols-4 gap-4 md:gap-6'>
                {[
                    { img: '/memory.png', name: 'Memory Match' },
                    { img: '/tic tac toe.jpg', name: 'Tic Tac Toe' },
                    { img: '/suduko.png', name: 'Sudoku Challenge' },
                    { img: '/zip.png', name: 'Zip Puzzle' }
                ].map((game, index) => (
                    <div key={index} className="relative group overflow-hidden rounded-xl bg-gray-800/50 backdrop-blur-sm border border-gray-700/50">
                        <img src={game.img} className='w-full object-cover transform transition-transform duration-500 group-hover:scale-110' alt={game.name} />
            ...
            <div className="mt-12">
                ...
                <div className="bg-[#1A1A1E] rounded-xl p-1 backdrop-blur-sm border border-gray-700/30">
                    {leaderboardData.map((player) => (
                        <div key={player.rank} className={`relative flex items-center justify-between p-4 rounded-lg mb-2 transition-all duration-300 ${player.rank === 1 ? 'bg-gradient-to-r from-amber-900/20 to-yellow-900/10 border border-amber-500/30' : 'bg-gray-800/20 hover:bg-gray-700/30'} ${isVisible ? 'opacity-100 translate-y-0' : 'opacity-0 translate-y-4'}`}
                        style={{ transitionDelay: `${player.rank * 100}ms` }}>
                            {player.rank === 1 && (
                                <div className="absolute -top-3 left-1/2 transform -translate-x-1/2 animate-bounce">
                                    <FaCrown className="text-yellow-500 text-2xl drop-shadow-[0_0_8px_rgba(255,215,0,0.6)]" />
                                </div>
                            )}
