  ...
  if (!companion) return (
    <div className="min-h-screen flex items-center justify-center ">
      <div className="text-center p-8 bg-gray-800/40 backdrop-blur-md rounded-2xl shadow-xl">
        ...
        <p className="text-gray-400">The companion you're looking for doesn't exist or may have been removed.</p>
      </div>
    </div>
  );
  ...
  return (
    <div className="min-h-screen  p-4 md:p-8">
      ...
      <div className="text-center mb-8 pt-4">
        <h1 className="text-transparent bg-clip-text bg-gradient-to-r from-green-400 to-cyan-400 font-extrabold text-5xl md:text-6xl uppercase mb-2 tracking-wide">
          {companion.name}
        </h1>
        <p className="text-gray-300 text-lg md:text-xl max-w-2xl mx-auto">
          Your AI assistant specialized in {companion.subject}
        </p>
      </div>
      ...
      <div className=" relative flex flex-col lg:flex-row gap-8 max-w-7xl mx-auto">
        ...
        <div className="w-full  flex flex-col">
          <div className="bg-gray-800/20 backdrop-blur-sm rounded-2xl p-6 shadow-xl shadow-violet-900/30 flex-1 border border-gray-700/50 flex flex-col">
          ...
          loop={true}
          className="w-[500px] h-[500px] mb-[-40px]"
        />
                
                {/* Status indicator */}
                <div className="absolute bottom-15 right-40">
                  <div className={`flex items-center justify-center w-10 h-10 rounded-full ${isSessionActive ? 'bg-green-500' : 'bg-gray-600'} shadow-lg`}>
                    {isSessionActive ? (
                      <div className="flex space-x-1">
                        <div className="w-1 h-3 bg-white rounded-full animate-audio-wave"></div>
                        <div className="w-1 h-4 bg-white rounded-full animate-audio-wave" style={{ animationDelay: '0.2s' }}></div>
                        <div className="w-1 h-5 bg-white rounded-full animate-audio-wave" style={{ animationDelay: '0.4s' }}></div>
                        <div className="w-1 h-4 bg-white rounded-full animate-audio-wave" style={{ animationDelay: '0.6s' }}></div>
                        <div className="w-1 h-3 bg-white rounded-full animate-audio-wave" style={{ animationDelay: '0.8s' }}></div>
                      </div>
                    ) : (
              ...
              <div className="text-center mb-6">
                <div className="inline-flex items-center gap-2 bg-violet-900/30 px-4 py-2 rounded-full mb-3">
                  <div className={`h-3 w-3 rounded-full ${isSessionActive ? 'bg-green-500 animate-pulse' : 'bg-amber-500'}`}></div>
                ...
                <div className="flex items-center justify-center gap-4 text-gray-300 mt-4">
                  <div className="flex items-center gap-2">
                    ...
                    <span className="text-sm capitalize">{companion.gender}</span>
                  </div>
                  ...
                  <div className="flex items-center gap-2">
                    ...
                    <span className="text-sm capitalize">{companion.language || 'English'}</span>
                  </div>
                </div>
              </div>
            </div>
            ...
            <button
              onClick={toggleCall}
              disabled={isLoading}
              className={`px-8 py-4 rounded-xl  font-semibold text-white transition-all duration-300 transform flex items-center justify-center gap-3 w-fit yop-5 right-5 absolute ${
                isSessionActive
            ...
            <div className="mt-6 p-4 bg-gray-800/30 rounded-xl border border-gray-700/30">
              ...
              <p className="text-gray-400 text-sm">
                {companion.description || `Expert in ${companion.subject} ready to help you with any questions or topics you'd like to discuss.`}
              </p>
            </div>
          </div>
        </div>
